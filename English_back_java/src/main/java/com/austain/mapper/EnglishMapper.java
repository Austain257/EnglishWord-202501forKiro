...
//    @Select("SELECT COUNT(*) FROM english_word_01 WHERE user_id IN (#{userId},0,1,2,3,4,5) AND is_grasp = 2")
    ...
    // 标记已掌握，在原数据表中修改字段
    @Update("UPDATE english_word_01 SET is_grasp = 1, update_time = NOW() where id = #{id}")
    int isGrasp(int id);
...
//    @Select("select * from english_word_01 where user_id in (#{userId},0,1,2,3,4,5) and book_id = #{bookId}")
    @Select("select * from english_word_01 where user_id = #{userId} and book_id = #{bookId}")
     ...
     * 更新单词中文释义
     */
    int updateChineseMeaning(@Param("id") Long id, @Param("chinese") String chinese);
