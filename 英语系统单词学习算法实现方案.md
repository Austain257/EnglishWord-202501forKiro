...
基于艾宾浩斯遗忘曲线理论，实现以**30分钟新词学习 + 两轮强制复习**为核心的最小记忆强化单元，通过状态锁机制确保用户严格遵循科学复习节奏。
...
#### 1. 单词学习记录表 `word_study_record`
```sql
CREATE TABLE `word_study_record` (  课本id没加进来
  `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键，自增。唯一标识每次学习记录',
  `user_id` bigint UNSIGNED NOT NULL COMMENT '用户id，外键，关联用户表',
  `start_time` datetime NOT NULL COMMENT '开始学习时间',
  `start_id` int UNSIGNED NOT NULL COMMENT '本轮学习单词范围的起始ID',
  `end_time` datetime NOT NULL COMMENT '实际结束时间',
  `end_id` int UNSIGNED NOT NULL COMMENT '本轮学习单词范围的结束ID',
  `status` tinyint UNSIGNED NOT NULL DEFAULT 1 COMMENT '会话状态：1-进行中，0-正常结束，4-异常退出',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录添加数据的时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录的最后一次修改时间',
  `round_1_review_time` datetime DEFAULT NULL COMMENT '第一轮复习结束的时间',
  `round_2_review_time` datetime DEFAULT NULL COMMENT '第二轮复习结束的时间',
  `round_3_review_time` datetime DEFAULT NULL COMMENT '第三轮复习结束的时间',
  `round_4_review_time` datetime DEFAULT NULL COMMENT '第四轮复习结束的时间',
  `round_5_review_time` datetime DEFAULT NULL COMMENT '第五轮复习结束的时间',
  `round_6_review_time` datetime DEFAULT NULL COMMENT '第六轮复习结束的时间',
  `round_7_review_time` datetime DEFAULT NULL COMMENT '第七轮复习结束的时间',
  `round_8_review_time` datetime DEFAULT NULL COMMENT '第八轮复习结束的时间',
  PRIMARY KEY (`id`),
...
#### 1.1 创建核心数据表
- [x] 学习清单表 `learning_checklist` (已存在)
- [ ] 单词学习记录表 `word_study_record` (需创建)
    ...
    read_loop: LOOP
        ...
        IF v_min_start_id IS NOT NULL AND v_max_end_id IS NOT NULL THEN
            INSERT INTO learning_checklist (
                ...
                v_user_id,
                CONCAT('学习新单词范围从', v_min_start_id, '到', v_max_end_id),
                1,
                0,
                0,
...
public class StudySessionRequest {
    private Long userId;
    private Integer startId;
    private Integer studyDuration; // 学习时长，单位秒，默认1800（30分钟）
}
...
public class ReviewCompleteRequest {
    ...
    private Integer reviewRound; // 复习轮次：1-8
...
**StudyTimer.vue**
```vue
<!-- 文件路径: src/components/learning/StudyTimer.vue -->
<template>
...
**RangeSelector.vue**
```vue
<!-- 文件路径: src/components/learning/RangeSelector.vue -->
<template>
...
const nextTenWords = async () => {
  ...
  try {
    ...
    success(`已切换到范围：${newRange.start}-${newRange.end}`)
  } catch (err) {
    error('切换范围失败：' + err.message)
  }
}
...
const applyRange = async () => {
  ...
  try {
    ...
    success(`已设置学习范围：${tempRange.value.start}-${tempRange.value.end}`)
  } catch (err) {
    error('设置学习范围失败：' + err.message)
  }
}
...
**StudyLockGuard.vue**
```vue
<!-- 文件路径: src/components/common/StudyLockGuard.vue -->
<template>
  <!-- 全局锁定遮罩 -->
...
Response:
{
  ...
  "data": {
    ...
    "message": "您有未完成的复习任务，请先完成第一、二轮复习！"
  }
}
```
...
<template>
  <div class="study-timer-container">
    ...
    <div class="timer-controls">
      ...
      <button class="end-btn">结束学习</button>
    </div>
    
    <!-- 学习进度 -->
    <div class="study-progress">
      <div class="progress-bar"></div>
      <div class="progress-text">已学习: 25/50 词</div>
    </div>
  </div>
</template>
```
  ...
  <div class="lock-overlay" v-if="isLocked">
    <div class="lock-modal">
      ...
      <div class="action-buttons">
        <button class="review-btn">进入复习中心</button>
        <button class="jottings-btn">使用积累本</button>
      </div>
    </div>
  </div>
</template>
  ...
  <div class="review-complete-section">
    <div class="review-progress">
      <div class="progress-steps">
        <div class="step completed">第一轮复习</div>
        <div class="step current">第二轮复习</div>
        <div class="step">解锁功能</div>
      </div>
    </div>
...
### 后端开发
- [ ] 创建word_study_record表
- [ ] 实现WordStudyController
- [ ] 实现StudyLockController
- [ ] 扩展StudyRecordController
- [ ] 实现相关Service层
- [ ] 实现相关Mapper层
- [ ] 添加定时任务
- [ ] 编写单元测试
...
本实现方案基于艾宾浩斯遗忘曲线理论，通过技术手段强制用户遵循科学的学习和复习节奏。核心特点：

1. **强制性学习流程**: 30分钟学习+两轮复习的闭环设计
2. **状态锁定机制**: 确保用户完成必要的复习任务
3. **数据驱动**: 完整的学习数据记录和分析
4. **用户体验**: 现代化的UI设计和响应式布局
5. **技术可靠**: 基于成熟的技术栈和最佳实践

通过分阶段实施，确保每个功能模块都经过充分测试和验证，最终形成一个完整、可靠的英语学习系统。
    ...
    <div class="bottom-controls">
      ...
      <div class="study-progress mb-4">
        <div class="text-sm text-slate-500 mb-2">
          本轮学习进度: {{ currentIndex + 1 }} / {{ totalWords }}
        </div>
  ...
  <div class="word-review-page">
    ...
    <div class="review-progress mb-6">
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm font-medium text-slate-600">第一轮复习进度</span>
...
const canComplete = computed(() => {
  return reviewProgress.value >= 80 // 至少完成80%才能标记完成
})
...
const markReviewComplete = async () => {
  try {
    await wordStudyStore.markReviewComplete({
      ...
      sessionId: getCurrentSessionId(), // 需要获取当前会话ID
      reviewRound: 1,
      completedTime: new Date()
    })
    
    success('第一轮复习已完成！')
    router.push('/word/review') // 跳转到复习选择页面
  } catch (err) {
    error('标记复习完成失败：' + err.message)
  }
}
</script>
```
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.meta.requiresAuth && authStore.isAuthenticated) {
    try {
      ...
      console.error('检查学习锁定状态失败:', error)
    }
  }
...
public class StudyTaskScheduler {
    ...
    public void generateDailyLearningChecklist() {
        System.out.println("开始生成每日学习清单...");
        try {
            wordStudyService.generateDailyChecklist();
            System.out.println("每日学习清单生成完成");
        } catch (Exception e) {
            System.err.println("生成每日学习清单失败: " + e.getMessage());
        }
    }
    ...
    public void checkTimeoutSessions() {
        System.out.println("检查超时学习会话...");
        try {
            wordStudyService.handleTimeoutSessions();
            System.out.println("超时会话处理完成");
        } catch (Exception e) {
            System.err.println("处理超时会话失败: " + e.getMessage());
        }
    }
}
```
...
**EnglishAndWordApplication.java 修改：**
```java
package com.austain;
...
#### 数据库层
- [ ] 创建 `word_study_record` 表
- [ ] 创建每日学习清单生成存储过程
- [ ] 创建定时任务触发器
- [ ] 测试数据库操作

#### 实体和DTO层
- [ ] 创建 `WordStudyRecord.java` 实体类
- [ ] 创建 `StudySessionRequest.java` 请求DTO
- [ ] 创建 `StudyEndRequest.java` 请求DTO
- [ ] 创建 `ReviewCompleteRequest.java` 请求DTO
- [ ] 创建 `StudyLockStatusResponse.java` 响应DTO

#### Mapper层
- [ ] 创建 `WordStudyMapper.java` 接口
- [ ] 实现学习记录CRUD操作
- [ ] 实现复习状态查询方法
- [ ] 实现锁定状态检查方法
- [ ] 编写对应的XML映射文件
...
#### Service层
- [ ] 创建 `WordStudyService.java` 接口
- [ ] 创建 `StudyLockService.java` 接口
- [ ] 实现 `WordStudyServiceImpl.java`
- [ ] 实现 `StudyLockServiceImpl.java`
- [ ] 实现业务逻辑和事务管理

#### Controller层
- [ ] 创建 `WordStudyController.java`
- [ ] 创建 `StudyLockController.java`
- [ ] 实现所有API端点
- [ ] 添加参数验证和异常处理

#### 定时任务
- [ ] 创建 `StudyTaskScheduler.java`
- [ ] 实现每日学习清单生成任务
- [ ] 实现超时会话处理任务
- [ ] 配置定时任务执行
...
#### 组件开发
- [ ] 创建 `StudyTimer.vue` 学习计时器组件
- [ ] 创建 `RangeSelector.vue` 范围选择组件
- [ ] 创建 `StudyLockGuard.vue` 锁定检查组件
- [ ] 创建 `ReviewProgress.vue` 复习进度组件

#### 状态管理
- [ ] 创建 `wordStudy.js` Store
- [ ] 实现学习会话状态管理
- [ ] 实现锁定状态管理
- [ ] 集成现有的 `word.js` Store

#### 服务层
- [ ] 创建 `wordStudy.service.js`
- [ ] 实现所有API调用方法
- [ ] 添加错误处理和重试机制
- [ ] 集成现有的API服务

#### 页面改造
- [ ] 改造 `WordLearning.vue` 页面
- [ ] 改造 `WordReview.vue` 页面
- [ ] 改造 `WordReviewSelection.vue` 页面
- [ ] 更新路由配置和守卫
...
1. **30分钟新词学习阶段**
   - ✅ 30分钟倒计时组件（圆形进度条，≥30dp尺寸）
   - ✅ 防误触机制（确认弹窗）
   - ✅ 学习范围选择（支持快捷按钮【下十个】）
   - ✅ 起始位置start_id和开始时间start_time记录
   - ✅ 倒计时结束提示和【标记并结束本轮学习】按钮
   - ✅ 强制性结束按钮（用户必须点击才能退出）
   - ✅ 结束位置end_id和结束时间end_time记录
   - ✅ word_study_record表数据插入
   - ✅ 鼓励性语言和复习提示

2. **状态锁定与功能禁用机制**
   - ✅ 触发条件检查（round_1_review_time IS NULL OR round_2_review_time IS NULL）
   - ✅ 全局警告弹窗
   - ✅ 主功能禁用（新词学习、句子练习、设置等）
   - ✅ 仅允许复习中心和积累本功能

3. **复习完成标记机制**
   - ✅ 第一轮和第二轮复习页面的【标记本轮复习已完成】按钮
   - ✅ round_1_review_time和round_2_review_time字段更新
   - ✅ 两轮复习完成后全局功能解锁

4. **数据库设计**
   - ✅ word_study_record表结构完全匹配需求
   - ✅ 所有必需字段和索引
   - ✅ 8轮复习字段预留
...
本实现方案完整覆盖了英语系统单词学习算法的所有核心需求：

### ✅ 已涵盖的功能
1. **30分钟新词学习机制** - 完整的计时器组件和会话管理
2. **两轮强制复习流程** - 复习状态跟踪和完成标记
3. **状态锁定机制** - 全局功能锁定和用户引导
4. **学习记录管理** - 完整的数据模型和API设计
5. **每日学习清单生成** - 定时任务和数据统计
6. **响应式UI设计** - 适配PC和移动端的现代化界面

### 🔧 技术特点
1. **前后端分离架构** - 清晰的API接口设计
2. **状态管理** - 基于Pinia的响应式状态管理
3. **数据持久化** - MySQL数据库和MyBatis ORM
4. **定时任务** - Spring Boot定时任务调度
5. **用户体验** - 现代化UI组件和交互设计

### 📈 扩展性考虑
1. **八轮复习支持** - 数据库预留8个复习字段
2. **多种复习模式** - 可扩展的复习类型配置
3. **个性化配置** - 可配置的学习时长和复习间隔
4. **数据分析** - 完整的学习数据收集和分析

通过分阶段实施，确保每个功能模块都经过充分测试和验证，最终形成一个完整、可靠、用户友好的英语学习系统。